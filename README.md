...
## cocoapod 私有库实践
cocopod 是一个第三方库依赖工具
![](https://note-1255605989.cosgz.myqcloud.com/zsOnj4Q0ro$$$cocoapod.png)


要完成一个pod私有化全过程，需要三个仓库(主项目仓库、pod库仓库、PodDemo项目仓库)、两个项目(主项目、PodDemo项目)
<!-- ![](img/cocoapod私有库流程.png) -->
![](https://note-1255605989.cosgz.myqcloud.com/Zwps4LB9fb$$$cocoapod私有库流程.png)
...
至此，说明pod库是可用的了。
<!-- ![](img/cocoapod本地仓库索引.png) -->

![](https://note-1255605989.cosgz.myqcloud.com/xkBItUsZ0k$$$cocoapod本地仓库索引.png)
...
* 把需要对外暴露的头文件在xx.h里暴露出来，否则会有警告

<!-- ![](img/framework.png) -->
![](https://note-1255605989.cosgz.myqcloud.com/rpaJ9M8TD5$$$framework.png)
...
#### 6、framework中使用到的资源问题
framework中使用到的资源，最好应该单独创建一个bundle，让bundle和framework分离，但是这样子使用的时候必须表明属于哪个bundle会恨蛋疼。否则就是资源还是放在main bundle里，不过这个独立demo就没法使用资源了。

### 8、framework存在的问题
* 因为framework只暴露了头文件，所以没法对其进行调试。这会是一个很蛋疼的问题
* 如果要达到framework最大优势化，应该给每个framework创建单独的测试和调试Demo
* 每个子模块也应该有对应的环境设置、登录信息等模块
* 如果不能自动打包，收到打包会恨繁琐，最好要有对应的工具链
...
## 项目处理
<!-- ![](img/App架构设计.png) -->
![](https://note-1255605989.cosgz.myqcloud.com/cdtV2B2S7n$$$App架构设计.png)
(卓明设计)
### 文件夹隔离
老的项目我们大多数是按单个小业务或者页面分文件夹，然后分成M、V、C。而模块化我们需要做如下几件事：
* 在就要求原来的基础上之上套一层模块文件夹;
* 将每一个模块除M、V、C之外的api、对外依赖、资源文件、宏也都分离出来;
* 将各个模块共享的文件、资源、宏整合到一起，沉淀到基础库里
* 模块间使用路由进行通信。controller之间尽量不依赖，相互通信尽可能走路由。通信之间尽可能减少信息流的传递

### 抽出基础库
在我们项目里，我们称之为BTCore。
可以看一下目前我们的BTCore有哪些东西：
```
Base            自定义basenavigation basecontroller等
Macro           宏定义 + 协议
ThirdParty      经过用源码导入的第三方 SDK
Kit             公用View + 所有工具类
Api             api模块
```
...
[路由设计思路分析](https://juejin.im/post/5ab85a346fb9a028e52dd16e)
